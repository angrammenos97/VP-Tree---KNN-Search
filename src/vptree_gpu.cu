...
#define MAXTHREADSPERBLOCK	128	//maximun threads per block
...
__device__ unsigned int NoP, DoP, vpMaxThreadsPerBlock;
//Buffers to build tree
__device__ double* treeMDs;
__device__ unsigned int* treeIDXs, * nodesOffset, * nodesLength;
								...
								unsigned int* d_nodeOffset, unsigned int* d_nodeLength, unsigned int numberOfPoints, unsigned int dimensionOfPoints, unsigned int maxThreadsPerBlock)
{
	if ((threadIdx.x) == 0 && (blockIdx.x == 0)) {
		NoP = numberOfPoints;	DoP = dimensionOfPoints;	vpMaxThreadsPerBlock = maxThreadsPerBlock;
		...
		treeMDs = d_treeMDs;		treeIDXs = d_treeIDXs;
		nodesOffset = d_nodeOffset;	nodesLength = d_nodeLength;
		nodesOffset[0] = 0;			nodesLength[0] = NoP;

		unsigned int blockSz = (NoP < vpMaxThreadsPerBlock) ? NoP : vpMaxThreadsPerBlock;
		unsigned int gridSz = (NoP + vpMaxThreadsPerBlock - 1) / vpMaxThreadsPerBlock;
		create_indexes_kernel <<<gridSz, blockSz>>> (indexesMain, NoP);
	}
}
...
__global__ void swap_points_kernel(double* out, double* in, unsigned int nodeOffset, unsigned int nodeLength)
{
	unsigned int tid = threadIdx.x + blockIdx.x * blockDim.x;
	if (tid < nodeLength)
		for (unsigned int d = 0; d < DoP; d++)
			*(out + tid + d * NoP) = *(in + vpSwaps[nodeOffset + tid] + d * NoP);
}
...
__global__ void vpt_kernel(unsigned int nodes, unsigned int maxNodes, unsigned int depth)
{
	...
	unsigned int tid = threadIdx.x + (blockIdx.x * blockDim.x);
	//Each thread creates it's assigned nodes
	for (unsigned int node = tid; node < nodes; node += MAXPARALLELNODES) {
		//Find out where the data is
		nodeOffset = nodesOffset[node * maxNodes / nodes];		nodeLength = nodesLength[node * maxNodes / nodes];
		if (!nodeLength) continue;		//if node doesn't exist continue to the next one
		if (depth & 1) {
		...
		treeIDXs[nodeOffset + (nodeLength - 1)] = indexesIn[nodeLength - 1];
		//Find the median of the distances
		qsOffset = node * ((2 * maxNodes) / nodes);
		treeMDs[nodeOffset + (nodeLength - 1)] = median(pointsIn, qsOffset, nodeLength, threadStream);
...
vptree* buildvp_gpu(double* points, int numberOfPoints, int dimensionOfPoints)
{
	...
	cudaError err = (cudaError)global_memory_allocate(numberOfPoints, dimensionOfPoints, MAXPARALLELNODES);
	if (err != cudaSuccess) { printf("Error: global_memory_allocate(%u, %u):%s\n", smallest_power_two(numberOfPoints), dimensionOfPoints, cudaGetErrorString(err)); return NULL; }
	...
	//Build tree
	unsigned int totalThreads, blockSz, gridSz, depth = 0, maxNodes = smallest_power_two(numberOfPoints + 1) / 2;
	cudaEvent_t start, stop;	cudaEventCreate(&start);	cudaEventCreate(&stop);
	cudaEventRecord(start);
	for (unsigned int nodes = 1; nodes <= maxNodes; nodes <<= 1) {
		depth += 1;
		totalThreads = (nodes < MAXPARALLELNODES) ? nodes : MAXPARALLELNODES;
		blockSz = (totalThreads < MAXTHREADSPERBLOCK) ? totalThreads : MAXTHREADSPERBLOCK;
		gridSz = (totalThreads + blockSz - 1) / blockSz;
	...
	printf("Calculations took %fmsec!\n ", milliseconds);
